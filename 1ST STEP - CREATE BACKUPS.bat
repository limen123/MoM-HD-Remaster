...
:NEXT
  ...
  IF exist "%CD:~0,2%\Program Files\KH_MoM\KINGDOM HEARTS Melody of Memory_Data\StreamingAssets\Sead\music\" (ECHO Window will automatically close when backups are created && ECHO Creating backups.. && >NUL copy "%CD:~0,2%\Program Files\KH_MoM\KINGDOM HEARTS Melody of Memory_Data\StreamingAssets\Sead\music\*.*" "Originals\") else (ECHO Default directory not found! && GOTO :NEXT2)
  GOTO END_CASE
:NEXT2
